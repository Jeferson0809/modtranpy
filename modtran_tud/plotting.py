...
def plot_standoff(res,
                  ...
                  figsize=(10, 6)):
    """
    Plot for standoff TUD result:
    - Path radiance along the line of sight (µflick)
    - LOS transmittance

    res : StandoffResult returned by run_standoff().
    """

    wl = res.wavelength
    Lp = res.path_radiance      # microflicks
    Tlos = res.transmittance    # 0–1

    fig, (ax_Lp, ax_T) = plt.subplots(
        2, 1, figsize=figsize, sharex=True
    )

    # --- Path radiance ---
    ax_Lp.plot(wl, Lp, color="purple", linewidth=0.8)
    ax_Lp.set_title("Standoff Path Radiance Lₚ (microflicks)")
    ax_Lp.set_ylabel("Radiance [µflick]")
    ax_Lp.grid(True, alpha=0.3)

    # --- Transmittance ---
    ax_T.plot(wl, Tlos, color="green", linewidth=0.8)
    ax_T.set_title("Standoff LOS Transmittance T(λ)")
    ax_T.set_xlabel("Wavelength (µm)")
    ax_T.set_ylabel("T(λ)")
    ax_T.set_ylim(0, 1)
    ax_T.grid(True, alpha=0.3)
